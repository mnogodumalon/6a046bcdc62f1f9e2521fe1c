...
import type { Test, CreateTest } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getTest(): Promise<Test[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.TEST}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Test[];
    return enrichLookupFields(records, 'test');
  }
  static async getTestEntry(id: string): Promise<Test | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.TEST}/records/${id}`);
    const record = { record_id: data.id, ...data } as Test;
    return enrichLookupFields([record], 'test')[0];
  }
  static async createTestEntry(fields: CreateTest) {
    return callApi('POST', `/apps/${APP_IDS.TEST}/records`, { fields: cleanFieldsForApi(fields as any, 'test') });
  }
  static async updateTestEntry(id: string, fields: Partial<CreateTest>) {
    return callApi('PATCH', `/apps/${APP_IDS.TEST}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'test') });
  }
  static async deleteTestEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.TEST}/records/${id}`);
  }
